...
Six areas on the specimen was sampled, and ther respective spectra are named: GaAs, Mo, (GaAs) NW, Si, Cu and Al.
...
\ton{Is the sentence above correct abou the NW? Is it "holey" with captial H?}
Si is the Si wafer.
Cu is the Cu-tape.
...
\brynjar{Fix citation}
% Ton: Can cite Oxford or manual. Avoid as much as possible web links, but here probably not to be avoid.
% Ton: 127 eV can serve as a reference to what you determine.Future work: does 80 mm^2 matter, can rather solid angle be measured?
% Ton done: remove "Solid angle" from "Solid angle detector"
The acceleration voltage, $V_{acc}$, can be set from 0.2 to 30 kV.
In this project the $V_{acc}$ was set to 5, 10, 15, and 30 kV.
The beam current, $I_{beam}$, has a maximum value of 400 nA.
In this project the $I_{beam}$ was set to 0.2, 0.4, 0.8, and 1.6 nA.
...
% AZtec
The data from AZtec was extraxted as described in Appendix A of Lundeby's master thesis \cite{lundeby_improving_2019}.
...
The end goal of this code is to improve understanding of the analysis process, and show how some of the steps can be done in different ways, e.g. calibration, peak finding and background estimation.
...
% Ton:
% These short section are too short for own header.
% If want to introduce, maybe have at the start of "1.6": "General data processing steps include i) converting to open data format and add/find ii) calibration. Further, standard processing include iii) fit and substract background, iV) fit characteristic peaks by Gaussian(s) and v) determine Gaussian characteristics representing peak position, width and counts under the peak. These steps are specified below.




% TODO: add transitions between the paragraphs
...
% Ton:
% This is discussion, not methods, keep things systematic in the right place.
...
% discussion: why is this? Just what scipy is optimized for?
% discussion: This was probably due to the fact that the function \verb|curve_fit| is not designed for spectra with a large dynamic range.
